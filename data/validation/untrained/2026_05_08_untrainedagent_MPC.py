"""
Real-plant deployment script for the untrained (baseline) RL-MPC agent on the batch distillation column.

Reads sensor states and current controls via OPC DA (WinCC gateway), filters
measurements with an EMA, and passes the augmented state to the untrained
RL_MPC_SPG_REINFORCE agent.  The agent's MPC solve returns the next control
action which is written back through OPC DA.

Used as baseline comparison against the trained agent run (see trained/2026_05_08_trainedagent_MPC.py).
...
class BatchColumn:
    def __init__(self):
        """Connect to OPC DA gateway and load the untrained RL-MPC agent."""
